...
const SWAP_AMOUNT: bigint = ethers.parseUnits('10', 'ether');

describe('ChainbrarySwapRouter', function () {
  const getPoolFromFactory = async (
    factory: ChainbrarySwapFactory,
    tokenA: CustomERC20Token,
    tokenB: CustomERC20Token,
  ...
  async function deployRouterWithNativeTokenFixture() {
    ...
    // Listen for the PoolCreated event and create a pool
    const tx: ContractTransactionResponse = await factory.createPool(ethers.ZeroAddress, tokenAAddress, FEE);
    const receipt: ContractTransactionReceipt | null = await tx.wait();
  ...
  it('should get an error message if reserve is empty', async () => {
    ...
    await expect(router.connect(addr1).getAmountsOut(amountIn, path, fees)).to.be.revertedWith('Invalid reserves');
  });
  ...
  it('should execute a token swap with a native token successfully', async () => {
    ...
    // Add liquidity to the pool
    const addLiquidityTx = await poolForAddr1
      .connect(addr1)
      .addLiquidity(liquidity[0], liquidity[1], { value: liquidity[0] });
    await addLiquidityTx.wait();
    ...
    // Swap from native token to ERC20 token
    const balanceBefore: bigint = await tokenA.balanceOf(addr2.address);
    expect(balanceBefore).to.be.equal(0);
    ...
    expect(poolBalance).to.be.equal(liquidity[0]);
    ...
    await router
      .connect(addr2)
      .swapExactTokensForTokens(halfAmountIn, amountOutMin, path, fees, addr2.address, { value: halfAmountIn });
  ...
  it.only('should execute a token swap from ERC20 to native token successfully', async () => {
    ...
    // Transfer tokens to addr1 and addr2 for liquidity and swap
    await tokenA.transfer(addr1.address, liquidity[0]);
    await tokenA.transfer(addr2.address, amountIn);
    await tokenA.connect(addr1).approve(poolAddress, liquidity[0]);
    await tokenA.connect(addr2).approve(routerAddress, amountIn);
    ...
    const addLiquidityTx = await poolForAddr1
      .connect(addr1)
      .addLiquidity(liquidity[0], liquidity[1], { value: liquidity[1] });
    await addLiquidityTx.wait();
    ...
    // Check balance of the pool
    const poolBalance = await ethers.provider.getBalance(poolAddress);
    expect(poolBalance).to.be.equal(liquidity[1]);
    ...
    // Get erc20 token balance before swap
    const balanceOfERC20TokenBefore: bigint = await tokenA.balanceOf(addr2.address);
    ...
    const balanceOfERC20TokenAfter: bigint = await tokenA.balanceOf(addr2.address);
    expect(balanceOfERC20TokenAfter).to.be.equal(balanceOfERC20TokenBefore - halfAmountIn);
  });
});
